...
Puesto que nos dan la matriz de disimilaridad, vamos a convertirla en una matatriz de distancias.
...
```{r}
rh.phylum.mds<-cmdscale(dist_VH,k=2)
```
...
```{r}
df_rh.phylum.mds = cbind(data.frame(rh.phylum.mds), as.factor(phylum))
colnames(df_rh.phylum.mds) = c("X", "Y", "phylum")
glimpse(df_rh.phylum.mds)
```

```{r}
plot(df_rh.phylum.mds$X, df_rh.phylum.mds$Y, pch = 19, col = factor(df_rh.phylum.mds$phylum), xlab = "Dim.1", ylab = "Dim.2")
legend("topleft",
       legend = levels(factor(df_rh.phylum.mds$phylum)),
       pch = 19,
       cex = 0.7,
       col = factor(levels(factor(df_rh.phylum.mds$phylum))))
```
