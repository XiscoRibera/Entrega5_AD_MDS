...
```{r}
vh.phylum.mds<-cmdscale(dist_VH,k=2)
```
...
```{r}
df_vh.phylum.mds = cbind(data.frame(vh.phylum.mds), as.factor(phylum))
colnames(df_vh.phylum.mds) = c("X", "Y", "phylum")
glimpse(df_vh.phylum.mds)
```

```{r}
plot(df_vh.phylum.mds$X, df_vh.phylum.mds$Y, pch = 19, col = factor(df_vh.phylum.mds$phylum), xlab = "Dim.1", ylab = "Dim.2")
legend("topleft",
       legend = levels(factor(df_vh.phylum.mds$phylum)),
       pch = 19,
       cex = 0.7,
       col = factor(levels(factor(df_vh.phylum.mds$phylum))))
```

```{r}
p <- ggplot(df_vh.phylum.mds, aes(x = X, y = Y, color = phylum)) +
  geom_point(size = 2.2) +
